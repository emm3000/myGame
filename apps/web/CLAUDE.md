...
- `src/fief/` — the fief screen at `/_signedIn/`: `FiefScreen` (presentational; one design-system `BuildingCard` per building), `buildingCardOf` (a card's costs and state from the building's `nextLevel` against the interpolated amounts and the free peasants, in the artifact's precedence: at max level, not enough peasants, too expensive, affordable; a disabled button's accessible name carries the reason), `liveFief` (interpolation from the last `FiefOverview` and its rates, capped at capacity; the slot countdown; the track total, the busy slot's `finishesAt - startedAt` in seconds, so the track is exact after a reload), `useLiveFief` (the re-read policy: on mount, on window focus, when the slot countdown reaches zero, and 60 seconds after the last read; `adopt` takes an overview answered by another call as a read) and `useUpgrade` (one enqueue in flight at a time, every card disabled meanwhile; success adopts the answered overview, a refusal shows under the refused card until the next read arrives). Its route tests live here as `fiefRoute.test.tsx` and `fiefUpgradeRoute.test.tsx`, on fake timers.
