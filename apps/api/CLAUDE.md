...
- `src/adapters/postgres/schema.ts` holds the Drizzle tables: `players`, `sessions`, `fiefs` (the five amounts, one `stored_at`, the nullable `slot_*` columns of the build slot, `slot_started_at` among them) and `fief_buildings`. No rate, capacity or peasant count is stored (ADR 005, ADR 007).
