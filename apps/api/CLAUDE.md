...
- `src/routes/fief.ts` also serves `POST /fief/upgrades` (200 `FiefOverview`) behind `requirePlayer`: the body parses with `EnqueueBuildingRequestSchema` (a failed parse answers `MalformedRequest`, 400). `src/fief/enqueueUpgradeOf.ts` runs `resolveUpgrade` then `enqueueBuilding` inside one `inTransaction` on the locked fief row at one frozen `now`, so a slot whose finish has passed is completed before the new upgrade starts, and a refusal rolls back the resolve too. The answered overview goes through `fiefOverviewOf`; each refusal goes through `answerRefusal`.
